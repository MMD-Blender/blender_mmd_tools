...
class OverwriteBoneMorphsFromActionPose(bpy.types.Operator):
    ...
    def execute(self, context):
        root = FnCore.find_root_object(context.active_object)
        FnMorph.overwrite_bone_morphs_from_pose_library(FnCore.find_armature_object(root))
...
class ApplyMaterialOffset(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        meshObj = FnCore.find_mesh_object_by_name(mat_data.related_mesh)
        if meshObj is None:
...
class ClearTempMaterials(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        root = FnCore.find_root_object(obj)
        assert root is not None
        for meshObj in FnCore.iterate_mesh_objects(root):
...
class ViewUVMorph(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        meshes = tuple(FnCore.iterate_mesh_objects(root))
        if len(meshes) == 1:
            obj = meshes[0]
...
class ClearUVMorphView(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        root = FnCore.find_root_object(obj)
        assert root is not None
        for m in FnCore.iterate_mesh_objects(root):
            mesh = m.data
...
class CleanDuplicatedMaterialMorphs(bpy.types.Operator):
    ...
    def execute(self, context: bpy.types.Context):
        mmd_root_object = FnCore.find_root_object(context.active_object)
